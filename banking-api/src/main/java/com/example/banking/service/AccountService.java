...
public class AccountService {
    ...
    public Account getByAccountNo(String accNo){
        return accounts.findByAccountNo(accNo).orElseThrow(()->new NotFoundException("Account not found"));
    }
    ...
    public AccountResponse withdraw(String accNo, BigDecimal amount){
        if(amount.signum() <= 0) throw new BadRequestException("Amount must be positive");
        var acc = getByAccountNo(accNo);
        if(acc.getBalance().compareTo(amount) < 0) throw new InsufficientFundsException("Insufficient balance");
    ...
    public AccountResponse transfer(String fromAccNo, TransferRequest req){
        if(fromAccNo.equals(req.getToAccountNo())) throw new BadRequestException("Cannot transfer to same account");
        if(req.getAmount().signum() <= 0) throw new BadRequestException("Amount must be positive");

        var from = getByAccountNo(fromAccNo);
        var to = getByAccountNo(req.getToAccountNo());

        if(from.getBalance().compareTo(req.getAmount()) < 0) throw new InsufficientFundsException("Insufficient balance");
        ...
        txRepo.save(new Transaction(from.getAccountNo(), TransactionType.TRANSFER_OUT, req.getAmount(), from.getBalance(),
                "Transfer to "+to.getAccountNo()));
        txRepo.save(new Transaction(to.getAccountNo(), TransactionType.TRANSFER_IN, req.getAmount(), to.getBalance(),
